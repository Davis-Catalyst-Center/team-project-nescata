...
uint16 CPU::getOperandAddress(AddressingMode mode) {
	...
	switch (mode) {
		...
		case RELATIVE: {
            // Relative address is calculated from the address of the *next* instruction.
            // PC has already been advanced past the opcode, so it's pointing at the offset.
		...
		case INDIRECT:
            // The famous indirect JMP bug is emulated here
			addr = readMem16(pc);
			pc += 2;
...
uint8 CPU::_toUnsigned(int8 val) {
	if (val < 0)
		return (~(-val) + 1) & 0xFF;
	else
		return val & 0xFF;
...
void CPU::_branch(bool condition) {
    // Branch instructions read a 1-byte signed offset.
    int8 offset = readMem(pc++);

	if (condition) {
		// A branch always takes at least +1 cycle if successful.
		cycles++;
        const uint16 target_addr = pc + offset;

		// An additional +1 cycle is taken if the branch crosses a page boundary.
...
void CPU::op_LDX(AddressingMode mode) {
	...
	x = readMem(addr);
	_setZNFlags(x);
    if (mode == ABSOLUTE_Y || mode == INDIRECT_Y) { // LDX uses Absolute,Y
        if (pageCrossed) cycles++;
    }
}
...
void CPU::op_LDY(AddressingMode mode) {
	...
	y = readMem(addr);
	_setZNFlags(y);
    if (mode == ABSOLUTE_X) { // LDY uses Absolute,X
        if (pageCrossed) cycles++;
    }
}
...
void CPU::op_NOP(AddressingMode mode) {
	// Some unofficial NOPs read operand bytes, so we must still
    // call getOperandAddress to advance the PC correctly.
    if (mode != IMPLIED && mode != ACCUMULATOR) {
        getOperandAddress(mode);

        // Unofficial NOPs with Absolute,X addressing add a cycle on page cross.
        // This affects opcodes 1C, 3C, 5C, 7C, DC, FC.
        if (mode == ABSOLUTE_X) {
            if (pageCrossed) {
                cycles++;
            }
...
void CPU::op_SHA(AddressingMode mode) {
	uint16 addr = getOperandAddress(mode);
	uint8 highByte = (addr >> 8) + 1;
	uint8 val = a & x & highByte;
	writeMem(addr, val);
}
...
void CPU::logInstruction(uint16 instrPc, uint8 opcode, const uint8* opcodeBytes, size_t byteCount) {
	...
	// Print registers in the original compact style
...
void CPU::runInstruction(uint8 opcode) {
	// Opcode dispatch. Refer to `cpu.hpp` UNOFFICIAL OPCODES comment for
	// details on unofficial/illegal opcodes and addressing-mode variants.
	switch (opcode) {
		...
		case 0x02: op_JAM(IMPLIED);      break; // JAM (0x02) | 1 | 0  | IMPLIED
		case 0x03: op_SLO(INDIRECT_X);   break; // SLO (0x03) | 2 | 8  | (indirect,X)
		case 0x04: op_NOP(ZERO_PAGE);    break; // NOP (0x04) | 2 | 3  | zeropage
		case 0x05: op_ORA(ZERO_PAGE);    break; // ORA (0x05) | 2 | 3  | zeropage
		case 0x06: op_ASL(ZERO_PAGE);    break; // ASL (0x06) | 2 | 5  | zeropage
		case 0x07: op_SLO(ZERO_PAGE);    break; // SLO (0x07) | 2 | 5  | zeropage
		...
		case 0x0b: op_ANC(IMMEDIATE);    break; // ANC (0x0B) | 2 | 2  | immediate
		case 0x0c: op_NOP(ABSOLUTE);     break; // NOP (0x0C) | 3 | 4  | absolute
		case 0x0d: op_ORA(ABSOLUTE);     break; // ORA (0x0D) | 3 | 4  | absolute
		case 0x0e: op_ASL(ABSOLUTE);     break; // ASL (0x0E) | 3 | 6  | absolute
		case 0x0f: op_SLO(ABSOLUTE);     break; // SLO (0x0F) | 3 | 6  | absolute

		case 0x10: op_BPL(RELATIVE);     break; // BPL (0x10) | 2 | 2**| relative
		case 0x11: op_ORA(INDIRECT_Y);   break; // ORA (0x11) | 2 | 5* | (indirect),Y
		case 0x12: op_JAM(IMPLIED);      break; // JAM (0x12) | 1 | 0  | IMPLIED
		case 0x13: op_SLO(INDIRECT_Y);   break; // SLO (0x13) | 2 | 8  | (indirect),Y
		case 0x14: op_NOP(ZERO_PAGE_X);  break; // NOP (0x14) | 2 | 4  | zeropage,X
		case 0x15: op_ORA(ZERO_PAGE_X);  break; // ORA (0x15) | 2 | 4  | zeropage,X
		case 0x16: op_ASL(ZERO_PAGE_X);  break; // ASL (0x16) | 2 | 6  | zeropage,X
		case 0x17: op_SLO(ZERO_PAGE_X);  break; // SLO (0x17) | 2 | 6  | zeropage,X
		case 0x18: op_CLC(IMPLIED);      break; // CLC (0x18) | 1 | 2  | implied
		case 0x19: op_ORA(ABSOLUTE_Y);   break; // ORA (0x19) | 3 | 4* | absolute,Y
		case 0x1a: op_NOP(IMPLIED);      break; // NOP (0x1A) | 1 | 2  | implied
		case 0x1b: op_SLO(ABSOLUTE_Y);   break; // SLO (0x1B) | 3 | 7  | absolute,Y
		case 0x1c: op_NOP(ABSOLUTE_X);   break; // NOP (0x1C) | 3 | 4* | absolute,X
		case 0x1d: op_ORA(ABSOLUTE_X);   break; // ORA (0x1D) | 3 | 4* | absolute,X
		case 0x1e: op_ASL(ABSOLUTE_X);   break; // ASL (0x1E) | 3 | 7  | absolute,X
		case 0x1f: op_SLO(ABSOLUTE_X);   break; // SLO (0x1F) | 3 | 7  | absolute,X

		case 0x20: op_JSR(ABSOLUTE);     break; // JSR (0x20) | 3 | 6  | absolute
		case 0x21: op_AND(INDIRECT_X);   break; // AND (0x21) | 2 | 6  | (indirect,X)
		case 0x22: op_JAM(IMPLIED);      break; // JAM (0x22) | 1 | 0  | IMPLIED
		case 0x23: op_RLA(INDIRECT_X);   break; // RLA (0x23) | 2 | 8  | (indirect,X)
		...
		case 0x27: op_RLA(ZERO_PAGE);    break; // RLA (0x27) | 2 | 5  | zeropage
		...
		case 0x2b: op_ANC2(IMMEDIATE);   break; // ANC (0x2B) | 2 | 2  | immediate
		...
		case 0x2f: op_RLA(ABSOLUTE);     break; // RLA (0x2F) | 3 | 6  | absolute

		case 0x30: op_BMI(RELATIVE);     break; // BMI (0x30) | 2 | 2**| relative
		case 0x31: op_AND(INDIRECT_Y);   break; // AND (0x31) | 2 | 5* | (indirect),Y
		case 0x32: op_JAM(IMPLIED);      break; // JAM (0x32) | 1 | 0  | IMPLIED
		case 0x33: op_RLA(INDIRECT_Y);   break; // RLA (0x33) | 2 | 8  | (indirect),Y
		case 0x34: op_NOP(ZERO_PAGE_X);  break; // NOP (0x34) | 2 | 4  | zeropage,X
		case 0x35: op_AND(ZERO_PAGE_X);  break; // AND (0x35) | 2 | 4  | zeropage,X
		case 0x36: op_ROL(ZERO_PAGE_X);  break; // ROL (0x36) | 2 | 6  | zeropage,X
		case 0x37: op_RLA(ZERO_PAGE_X);  break; // RLA (0x37) | 2 | 6  | zeropage,X
		case 0x38: op_SEC(IMPLIED);      break; // SEC (0x38) | 1 | 2  | implied
		case 0x39: op_AND(ABSOLUTE_Y);   break; // AND (0x39) | 3 | 4* | absolute,Y
		case 0x3a: op_NOP(IMPLIED);      break; // NOP (0x3A) | 1 | 2  | implied
		case 0x3b: op_RLA(ABSOLUTE_Y);   break; // RLA (0x3B) | 3 | 7  | absolute,Y
		case 0x3c: op_NOP(ABSOLUTE_X);   break; // NOP (0x3C) | 3 | 4* | absolute,X
		case 0x3d: op_AND(ABSOLUTE_X);   break; // AND (0x3D) | 3 | 4* | absolute,X
		case 0x3e: op_ROL(ABSOLUTE_X);   break; // ROL (0x3E) | 3 | 7  | absolute,X
		case 0x3f: op_RLA(ABSOLUTE_X);   break; // RLA (0x3F) | 3 | 7  | absolute,X

		case 0x40: op_RTI(IMPLIED);      break; // RTI (0x40) | 1 | 6  | implied
		case 0x41: op_EOR(INDIRECT_X);   break; // EOR (0x41) | 2 | 6  | (indirect,X)
		case 0x42: op_JAM(IMPLIED);      break; // JAM (0x42) | 1 | 0  | IMPLIED
		case 0x43: op_SRE(INDIRECT_X);   break; // SRE (0x43) | 2 | 8  | (indirect,X)
		case 0x44: op_NOP(ZERO_PAGE);    break; // NOP (0x44) | 2 | 3  | zeropage
		case 0x45: op_EOR(ZERO_PAGE);    break; // EOR (0x45) | 2 | 3  | zeropage
		case 0x46: op_LSR(ZERO_PAGE);    break; // LSR (0x46) | 2 | 5  | zeropage
		case 0x47: op_SRE(ZERO_PAGE);    break; // SRE (0x47) | 2 | 5  | zeropage
		...
		case 0x4b: op_ALR(IMMEDIATE);    break; // ALR (0x4B) | 2 | 2  | immediate
		...
		case 0x4f: op_SRE(ABSOLUTE);     break; // SRE (0x4F) | 3 | 6  | absolute

		case 0x50: op_BVC(RELATIVE);     break; // BVC (0x50) | 2 | 2**| relative
		case 0x51: op_EOR(INDIRECT_Y);   break; // EOR (0x51) | 2 | 5* | (indirect),Y
		case 0x52: op_JAM(IMPLIED);      break; // JAM (0x52) | 1 | 0  | IMPLIED
		case 0x53: op_SRE(INDIRECT_Y);   break; // SRE (0x53) | 2 | 8  | (indirect),Y
		case 0x54: op_NOP(ZERO_PAGE_X);  break; // NOP (0x54) | 2 | 4  | zeropage,X
		case 0x55: op_EOR(ZERO_PAGE_X);  break; // EOR (0x55) | 2 | 4  | zeropage,X
		case 0x56: op_LSR(ZERO_PAGE_X);  break; // LSR (0x56) | 2 | 6  | zeropage,X
		case 0x57: op_SRE(ZERO_PAGE_X);  break; // SRE (0x57) | 2 | 6  | zeropage,X
		case 0x58: op_CLI(IMPLIED);      break; // CLI (0x58) | 1 | 2  | implied
		case 0x59: op_EOR(ABSOLUTE_Y);   break; // EOR (0x59) | 3 | 4* | absolute,Y
		case 0x5a: op_NOP(IMPLIED);      break; // NOP (0x5A) | 1 | 2  | implied
		case 0x5b: op_SRE(ABSOLUTE_Y);   break; // SRE (0x5B) | 3 | 7  | absolute,Y
		case 0x5c: op_NOP(ABSOLUTE_X);   break; // NOP (0x5C) | 3 | 4* | absolute,X
		case 0x5d: op_EOR(ABSOLUTE_X);   break; // EOR (0x5D) | 3 | 4* | absolute,X
		case 0x5e: op_LSR(ABSOLUTE_X);   break; // LSR (0x5E) | 3 | 7  | absolute,X
		case 0x5f: op_SRE(ABSOLUTE_X);   break; // SRE (0x5F) | 3 | 7  | absolute,X

		case 0x60: op_RTS(IMPLIED);      break; // RTS (0x60) | 1 | 6  | implied
		case 0x61: op_ADC(INDIRECT_X);   break; // ADC (0x61) | 2 | 6  | (indirect,X)
		case 0x62: op_JAM(IMPLIED);      break; // JAM (0x62) | 1 | 0  | IMPLIED
		case 0x63: op_RRA(INDIRECT_X);   break; // RRA (0x63) | 2 | 8  | (indirect,X)
		case 0x64: op_NOP(ZERO_PAGE);    break; // NOP (0x64) | 2 | 3  | zeropage
		case 0x65: op_ADC(ZERO_PAGE);    break; // ADC (0x65) | 2 | 3  | zeropage
		case 0x66: op_ROR(ZERO_PAGE);    break; // ROR (0x66) | 2 | 5  | zeropage
		case 0x67: op_RRA(ZERO_PAGE);    break; // RRA (0x67) | 2 | 5  | zeropage
		...
		case 0x6b: op_ARR(IMMEDIATE);    break; // ARR (0x6B) | 2 | 2  | immediate
		...
		case 0x6f: op_RRA(ABSOLUTE);     break; // RRA (0x6F) | 3 | 6  | absolute

		case 0x70: op_BVS(RELATIVE);     break; // BVS (0x70) | 2 | 2**| relative
		case 0x71: op_ADC(INDIRECT_Y);   break; // ADC (0x71) | 2 | 5* | (indirect),Y
		case 0x72: op_JAM(IMPLIED);      break; // JAM (0x72) | 1 | 0  | IMPLIED
		case 0x73: op_RRA(INDIRECT_Y);   break; // RRA (0x73) | 2 | 8  | (indirect),Y
		case 0x74: op_NOP(ZERO_PAGE_X);  break; // NOP (0x74) | 2 | 4  | zeropage,X
		case 0x75: op_ADC(ZERO_PAGE_X);  break; // ADC (0x75) | 2 | 4  | zeropage,X
		case 0x76: op_ROR(ZERO_PAGE_X);  break; // ROR (0x76) | 2 | 6  | zeropage,X
		case 0x77: op_RRA(ZERO_PAGE_X);  break; // RRA (0x77) | 2 | 6  | zeropage,X
		case 0x78: op_SEI(IMPLIED);      break; // SEI (0x78) | 1 | 2  | implied
		case 0x79: op_ADC(ABSOLUTE_Y);   break; // ADC (0x79) | 3 | 4* | absolute,Y
		case 0x7a: op_NOP(IMPLIED);      break; // NOP (0x7A) | 1 | 2  | implied
		case 0x7b: op_RRA(ABSOLUTE_Y);   break; // RRA (0x7B) | 3 | 7  | absolute,Y
		case 0x7c: op_NOP(ABSOLUTE_X);   break; // NOP (0x7C) | 3 | 4* | absolute,X
		case 0x7d: op_ADC(ABSOLUTE_X);   break; // ADC (0x7D) | 3 | 4* | absolute,X
		case 0x7e: op_ROR(ABSOLUTE_X);   break; // ROR (0x7E) | 3 | 7  | absolute,X
		case 0x7f: op_RRA(ABSOLUTE_X);   break; // RRA (0x7F) | 3 | 7  | absolute,X

		case 0x80: op_NOP(IMMEDIATE);    break; // NOP (0x80) | 2 | 2  | immediate
		case 0x81: op_STA(INDIRECT_X);   break; // STA (0x81) | 2 | 6  | (indirect,X)
		case 0x82: op_NOP(IMMEDIATE);    break; // NOP (0x82) | 2 | 2  | immediate
		case 0x83: op_SAX(INDIRECT_X);   break; // SAX (0x83) | 2 | 6  | (indirect,X)
		...
		case 0x87: op_SAX(ZERO_PAGE);    break; // SAX (0x87) | 2 | 3  | zeropage
		case 0x88: op_DEY(IMPLIED);      break; // DEY (0x88) | 1 | 2  | implied
		case 0x89: op_NOP(IMMEDIATE);    break; // NOP (0x89) | 2 | 2  | immediate
		case 0x8a: op_TXA(IMPLIED);      break; // TXA (0x8A) | 1 | 2  | implied
		case 0x8b: op_ANE(IMMEDIATE);    break; // ANE (0x8B) | 2 | 2  | immediate (unstable)
		...
		case 0x8f: op_SAX(ABSOLUTE);     break; // SAX (0x8F) | 3 | 4  | absolute

		case 0x90: op_BCC(RELATIVE);     break; // BCC (0x90) | 2 | 2**| relative
		case 0x91: op_STA(INDIRECT_Y);   break; // STA (0x91) | 2 | 6  | (indirect),Y
		case 0x92: op_JAM(IMPLIED);      break; // JAM (0x92) | 1 | 0  | IMPLIED
		case 0x93: op_SHA(INDIRECT_Y);   break; // SHA (0x93) | 2 | 6  | (indirect),Y
		...
		case 0x97: op_SAX(ZERO_PAGE_Y);  break; // SAX (0x97) | 2 | 4  | zeropage,Y
		...
		case 0x9b: op_TAS(ABSOLUTE_Y);   break; // TAS (0x9B) | 3 | 5  | absolute,Y
		case 0x9c: op_SHY(ABSOLUTE_X);   break; // SHY (0x9C) | 3 | 5  | absolute,X
		case 0x9d: op_STA(ABSOLUTE_X);   break; // STA (0x9D) | 3 | 5  | absolute,X
		case 0x9e: op_SHX(ABSOLUTE_Y);   break; // SHX (0x9E) | 3 | 5  | absolute,Y
		case 0x9f: op_SHA(ABSOLUTE_Y);   break; // SHA (0x9F) | 3 | 5  | absolute,Y
		...
		case 0xa3: op_LAX(INDIRECT_X);   break; // LAX (0xA3) | 2 | 6  | (indirect,X)
		...
		case 0xa7: op_LAX(ZERO_PAGE);    break; // LAX (0xA7) | 2 | 3  | zeropage
		...
		case 0xab: op_LXA(IMMEDIATE);    break; // LXA (0xAB) | 2 | 2  | immediate (unstable)
		...
		case 0xaf: op_LAX(ABSOLUTE);     break; // LAX (0xAF) | 3 | 4  | absolute

		case 0xb0: op_BCS(RELATIVE);     break; // BCS (0xB0) | 2 | 2**| relative
		case 0xb1: op_LDA(INDIRECT_Y);   break; // LDA (0xB1) | 2 | 5* | (indirect),Y
		case 0xb2: op_JAM(IMPLIED);      break; // JAM (0xB2) | 1 | 0  | IMPLIED
		case 0xb3: op_LAX(INDIRECT_Y);   break; // LAX (0xB3) | 2 | 5* | (indirect),Y
		...
		case 0xb7: op_LAX(ZERO_PAGE_Y);  break; // LAX (0xB7) | 2 | 4  | zeropage,Y
		...
		case 0xbb: op_LAS(ABSOLUTE_Y);   break; // LAS (0xBB) | 3 | 4* | absolute,Y
		...
		case 0xbf: op_LAX(ABSOLUTE_Y);   break; // LAX (0xBF) | 3 | 4* | absolute,Y

		case 0xc0: op_CPY(IMMEDIATE);    break; // CPY (0xC0) | 2 | 2  | immediate
		case 0xc1: op_CMP(INDIRECT_X);   break; // CMP (0xC1) | 2 | 6  | (indirect,X)
		case 0xc2: op_NOP(IMMEDIATE);    break; // NOP (0xC2) | 2 | 2  | immediate (double/variable NOP)
		case 0xc3: op_DCP(INDIRECT_X);   break; // DCP (0xC3) | 2 | 8  | (indirect,X)
		...
		case 0xc7: op_DCP(ZERO_PAGE);    break; // DCP (0xC7) | 2 | 5  | zeropage
		...
		case 0xcb: op_SBX(IMMEDIATE);    break; // SBX (0xCB) | 2 | 2  | immediate
		...
		case 0xcf: op_DCP(ABSOLUTE);     break; // DCP (0xCF) | 3 | 6  | absolute

		case 0xd0: op_BNE(RELATIVE);     break; // BNE (0xD0) | 2 | 2**| relative
		case 0xd1: op_CMP(INDIRECT_Y);   break; // CMP (0xD1) | 2 | 5* | (indirect),Y
		case 0xd2: op_JAM(IMPLIED);      break; // JAM (0xD2) | 1 | 0  | IMPLIED
		case 0xd3: op_DCP(INDIRECT_Y);   break; // DCP (0xD3) | 2 | 8  | (indirect),Y
		case 0xd4: op_NOP(ZERO_PAGE_X);  break; // NOP (0xD4) | 2 | 4  | zeropage,X
		case 0xd5: op_CMP(ZERO_PAGE_X);  break; // CMP (0xD5) | 2 | 4  | zeropage,X
		case 0xd6: op_DEC(ZERO_PAGE_X);  break; // DEC (0xD6) | 2 | 6  | zeropage,X
		case 0xd7: op_DCP(ZERO_PAGE_X);  break; // DCP (0xD7) | 2 | 6  | zeropage,X
		case 0xd8: op_CLD(IMPLIED);      break; // CLD (0xD8) | 1 | 2  | implied
		case 0xd9: op_CMP(ABSOLUTE_Y);   break; // CMP (0xD9) | 3 | 4* | absolute,Y
		case 0xda: op_NOP(IMPLIED);      break; // NOP (0xDA) | 1 | 2  | implied
		case 0xdb: op_DCP(ABSOLUTE_Y);   break; // DCP (0xDB) | 3 | 7  | absolute,Y
		case 0xdc: op_NOP(ABSOLUTE_X);   break; // NOP (0xDC) | 3 | 4* | absolute,X
		case 0xdd: op_CMP(ABSOLUTE_X);   break; // CMP (0xDD) | 3 | 4* | absolute,X
		case 0xde: op_DEC(ABSOLUTE_X);   break; // DEC (0xDE) | 3 | 7  | absolute,X
		case 0xdf: op_DCP(ABSOLUTE_X);   break; // DCP (0xDF) | 3 | 7  | absolute,X

		case 0xe0: op_CPX(IMMEDIATE);    break; // CPX (0xE0) | 2 | 2  | immediate
		case 0xe1: op_SBC(INDIRECT_X);   break; // SBC (0xE1) | 2 | 6  | (indirect,X)
		case 0xe2: op_NOP(IMMEDIATE);    break; // NOP (0xE2) | 2 | 2  | immediate
		case 0xe3: op_ISC(INDIRECT_X);   break; // ISC (0xE3) | 2 | 8  | (indirect,X)
		...
		case 0xe7: op_ISC(ZERO_PAGE);    break; // ISC (0xE7) | 2 | 5  | zeropage
		...
		case 0xeb: op_USBC(IMMEDIATE);   break; // USBC (0xEB)| 2 | 2  | immediate (SBC variant)
		...
		case 0xef: op_ISC(ABSOLUTE);     break; // ISC (0xEF) | 3 | 6  | absolute

		case 0xf0: op_BEQ(RELATIVE);     break; // BEQ (0xF0) | 2 | 2**| relative
		case 0xf1: op_SBC(INDIRECT_Y);   break; // SBC (0xF1) | 2 | 5* | (indirect),Y
		case 0xf2: op_NOP(IMPLIED);      break; // NOP (0xF2) | 1 | 0  | IMPLIED
		case 0xf3: op_ISC(INDIRECT_Y);   break; // ISC (0xF3) | 2 | 8  | (indirect),Y
		case 0xf4: op_NOP(ZERO_PAGE_X);  break; // NOP (0xF4) | 2 | 4  | zeropage,X
		case 0xf5: op_SBC(ZERO_PAGE_X);  break; // SBC (0xF5) | 2 | 4  | zeropage,X
		case 0xf6: op_INC(ZERO_PAGE_X);  break; // INC (0xF6) | 2 | 6  | zeropage,X
		case 0xf7: op_ISC(ZERO_PAGE_X);  break; // ISC (0xF7) | 2 | 6  | zeropage,X
		case 0xf8: op_SED(IMPLIED);      break; // SED (0xF8) | 1 | 2  | implied
		case 0xf9: op_SBC(ABSOLUTE_Y);   break; // SBC (0xF9) | 3 | 4* | absolute,Y
		case 0xfa: op_NOP(IMPLIED);      break; // NOP (0xFA) | 1 | 2  | implied
		case 0xfb: op_ISC(ABSOLUTE_Y);   break; // ISC (0xFB) | 3 | 7  | absolute,Y
		case 0xfc: op_NOP(ABSOLUTE_X);   break; // NOP (0xFC) | 3 | 4* | absolute,X
		case 0xfd: op_SBC(ABSOLUTE_X);   break; // SBC (0xFD) | 3 | 4* | absolute,X
		case 0xfe: op_INC(ABSOLUTE_X);   break; // INC (0xFE) | 3 | 7  | absolute,X
		case 0xff: op_ISC(ABSOLUTE_X);   break; // ISC (0xFF) | 3 | 7  | absolute,X
